...
import com.bxgis.bxportal.bean.ProjectFilterTreeRoot;
...
public class ProjectSelectActivity extends BaseActivity implements View.OnClickListener,FilterTreeView.OnItemClickListener{
    @Bind(R.id.tv_back)
    TextView back;
    @Bind(R.id.tv_ok)
    TextView Ok;
    ...
    ProjectFilterTreeRoot mFilterRoot ;
    List<InspectionSubProjectBean> datas = new ArrayList<>();
    List<InspectionSubProjectBean> datas2 = new ArrayList<>();
...
//    }
    ...
    protected void initView() {
        ...
        datas = (List<InspectionSubProjectBean>) getIntent().getSerializableExtra("PROJECTlIST");
...
//        TestFilterRoot2 mFilterRoot =new TestFilterRoot2();
        mFilterRoot = new ProjectFilterTreeRoot(datas);
                ...
                if(null!=mFilterTreeView){
                    ...
                    if(null!=mList&&mList.size()>0){
                        for(int a=0;a<mList.size();a++){
                            datas2.add((InspectionSubProjectBean) mList.get(a).getData());
                        }
                        Intent intent=new Intent();
                        intent.putExtra("SelectSubPro", (Serializable) datas2);
                        setResult(RESULT_OK,intent);
                        finish();
                    }else{
                        Toast.makeText(mContext, "请先选择巡检事项", Toast.LENGTH_SHORT).show();
                        return;
                    }
                }
//                mFilterTreeView.
