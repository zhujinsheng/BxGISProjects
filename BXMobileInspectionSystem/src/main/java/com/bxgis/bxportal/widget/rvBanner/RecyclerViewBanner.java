package com.bxgis.bxportal.widget.rvBanner;
...
public class RecyclerViewBanner extends FrameLayout {
    ...
    private void createIndicators() {
        ...
        for (int i = 0; i < mData.size(); i++) {
            ...
            if (mSize >= dp2px(6)) { // 设置了indicatorSize属性
                lp.width = lp.height = mSize;
            } else {
                // 如果设置的resource.xml没有明确的宽高，默认最小2dp，否则太小看不清
                img.setMinimumWidth(dp2px(5));
                img.setMinimumHeight(dp2px(5));
            }
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                boolean hasMoved = 1.5 * Math.abs(disX) > Math.abs(disY);
                getParent().getParent().requestDisallowInterceptTouchEvent(hasMoved);
                if (hasMoved) {
                    setPlaying(false);
                }
                break;
