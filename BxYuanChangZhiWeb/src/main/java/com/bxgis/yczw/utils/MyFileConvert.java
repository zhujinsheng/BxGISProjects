...
public class MyFileConvert implements Converter<File> {
    public static final String DM_TARGET_FOLDER = File.separator + "maintenance" + File.separator+"downFile"+File.separator; //下载目标文件夹

    private String folder;                  //目标文件存储的文件夹路径
    private String fileName;                //目标文件存储的文件名
    private Callback<File> callback;        //下载回调

    public MyFileConvert() {
        this(null);
    }

    public MyFileConvert(String fileName) {
        this(Environment.getExternalStorageDirectory() + DM_TARGET_FOLDER, fileName);
    }

    public MyFileConvert(String folder, String fileName) {
        this.folder = folder;
...
//       File file= new File(FileProviderUtils.getUriForFile(MISystemApplication.getContext(),file1).getPath());
